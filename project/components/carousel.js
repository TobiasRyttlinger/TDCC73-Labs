...
import {SafeAreaView,StyleSheet,View,Text,TextInput,Dimensions,FlatList,Image} from 'react-native';
...
const CarouselApp = ({carouselLenght, pageScroll, horizontal, spacing, imageHeightScale, imageWidthScale, imageInput}) => {
 ...
 //Generate an array of dummy images
  const imageList = Array.from({ length: carouselLenght }).map((notUsed, i) => {
  ...
  function SwipeFun({ inData }) {
    return (
      <View style={styles.swiper}>
        <Image
          source={{ uri: inData.image }}
  ...
  return (
      <SafeAreaView>
        <FlatList
          ...
          renderItem={({ item }) => {
            return <SwipeFun inData={item} />; //Reaching each view containing an image.
          }}
        />
      </SafeAreaView>
  );
